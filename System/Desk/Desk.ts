...
export class Desk {
    ...
    //	Context Menu
    showContextMenu(list, delegate, x, y) {
    ...
    setUpContextMenu(body, delegate) {
    ...
    startDrag(clipboard, view, x, y, originalX, originalY) {
        var i = 0;
    ...
    */
    ...
    closeWindow(window) {
        if(window == this.deskInstance.currentWindow)
            this.deskInstance.currentWindow = null;
        window.delete();
        window = null;
    }
    ...
    bringWindowFront(window) {
        ...
        if(window == this.deskInstance.currentWindow)
            return false;
        if(this.deskInstance.currentWindow && !this.deskInstance.currentWindow.deleted)
        this.deskInstance.currentWindow.putInSleep();
    ...
    addPluginFrame(frame) {
        frame.style.display = "none";
        document.body.appendChild(frame);
    }

    loadLogoAsInline() { // load logo as inline svg file
